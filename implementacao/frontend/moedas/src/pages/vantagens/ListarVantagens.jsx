...
const ListarVantagens = () => {
	...
	const filter = (arr, searchValue, value) => {
		if (searchValue) arr = arr.filter(elem => elem.nome.toLowerCase().includes(searchValue.toLowerCase()))

		return arr.filter(elem => elem.preco >= value[0] && elem.preco <= value[1])
	}
			...
			<section className='title-section flex space-between column-gap-3rem align-center'>
				<div className='section-1 flex flex-column row-gap-1rem'>
					...
					<p className='description-listar-vantagem'>
						Troque suas moedas aculumadas por vantagens como desconto em restaurantes da universidade,
						desconto de mensalidade, ou compra de materiais específicos.
					</p>

					<div>
						<Searchbar placeholder='Digite uma vantagem...' onChange={handleChangeSearchValue} />
					</div>
				</div>
				<img src='/bag.svg' alt='Troque moedas por vantagens' className='bag-img' />
			</section>
					...
					<>
						...
						<section className='cards flex flex-wrap column-gap-3rem row-gap-3rem space-between'>
							{filter(vantagens, searchValue, value).map(vantagem => (
								<CardVantagem key={Math.random()} content={vantagem} onClick={handleOnClick} />
							))}
						</section>
					</>
				)}
